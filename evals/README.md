...
Run each condition into the same results file. Candidate and comparator instructions are injected from the supplied instruction file; task prompts remain identical.
...
python3 scripts/run_evals.py run \
  --runner claude \
  --condition candidate \
  --condition-skill output-styles/i-have-adhd.md \
  --trials 3 \
  --budget-usd 12.50 \
  --output evals/results/responses.jsonl
```
...
The example runner isolates the call from the operator's own agent configuration with `--setting-sources ""`. Keep that isolation when adding runners: without it, user-level plugins, hooks, memory, and output styles leak into every condition and shape the responses being judged. The sharpest case is this repo's own plugin, whose forced output style would inject the full i-have-adhd ruleset into the **baseline** condition and make the comparison measure the ruleset against itself.
